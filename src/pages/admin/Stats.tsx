        ...
        >
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-8 md:pb-12 pt-6 md:pt-0">
                <div className="space-y-3 md:space-y-4 text-center sm:text-left">
                    ...
                    <div>
                        <h2 className="text-4xl sm:text-6xl md:text-8xl font-display font-black text-white uppercase tracking-tighter leading-[0.9] md:leading-[0.8] drop-shadow-2xl">
                            Command <br /> <span className="text-padel-yellow drop-shadow-[0_0_40px_rgba(255,210,31,0.2)]">& Performance</span>
                        </h2>
                        <p className="text-[10px] md:text-xs font-black text-white/30 uppercase tracking-[0.2em] md:tracking-[0.4em] mt-4 md:mt-6 flex items-center justify-center sm:justify-start gap-2 md:gap-3">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                    <div className="w-full sm:w-auto px-6 py-4 bg-white/[0.03] border border-white/10 rounded-xl md:rounded-[1.5rem] flex flex-col items-center sm:items-end">
                        <span className="text-[8px] md:text-[9px] font-black text-white/20 uppercase tracking-widest">Status Système</span>
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] md:text-[11px] font-black text-white tracking-tighter">SYNCHRONISÉ</span>
                        </div>
                    </div>
                    <button
                        onClick={fetchStats}
                    ...
                    >
                        <div className="flex items-center justify-between mb-8 md:mb-10">
                            ...
                            <div className="flex flex-col items-end">
                                <span className="text-[8px] md:text-[10px] font-black text-green-500 uppercase tracking-widest">{metric.change}</span>
                        ...
                        <div>
                            <p className="text-[8px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.2em] md:tracking-[0.35em] mb-1.5 md:mb-2">{metric.label}</p>
                            <p className="text-3xl md:text-5xl font-black text-white tracking-tighter leading-none">{metric.val}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-12 relative overflow-hidden lg:h-[540px] flex flex-col group shadow-3xl">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-12 sm:mb-16 relative z-10">
                        <div className="space-y-1">
                            <h3 className="text-xl md:text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-3 md:gap-4">
                                <BarChart3 className="text-padel-blue md:w-[28px] md:h-[28px]" size={22} /> Infrastructure
                            </h3>
                            <p className="text-[8px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.3em] md:tracking-[0.4em] ml-9 md:ml-11">Analyse des flux horaires</p>
                        </div>
                        ...
                        {(stats?.hourlyStats || [40, 65, 45, 90, 100, 75, 60, 85, 95, 70, 55, 80]).map((h, i) => (
                            <div key={i} className="flex-1 min-w-[25px] sm:min-w-0 flex flex-col items-center gap-4 md:gap-6 group/bar relative">
                                <div className="absolute -top-10 opacity-0 group-hover/bar:opacity-100 transition-all duration-300 pointer-events-none z-20">
                                    <div className="bg-white px-2 py-1 rounded text-padel-blue text-[8px] font-black shadow-2xl whitespace-nowrap">{h}%</div>
                                </div>
                                <motion.div
                                    initial={{ height: 0 }}
                                ...
                                <span className="text-[7px] md:text-[9px] font-black text-white/20 uppercase tracking-widest group-hover/bar:text-white transition-colors">{10 + i}h</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="mt-8 lg:absolute lg:bottom-12 lg:right-12 p-6 md:p-8 bg-white/[0.03] border border-white/10 rounded-2xl md:rounded-[2.5rem] backdrop-blur-3xl transform lg:group-hover:-translate-y-2 transition-all duration-700 shadow-2xl">
                        <div className="flex items-center gap-4 mb-3 md:mb-4">
                            ...
                            <div>
                                <p className="text-[8px] md:text-[9px] font-black text-padel-yellow uppercase tracking-widest">Peak</p>
                                <p className="text-xl md:text-3xl font-black text-white tracking-tighter">{stats?.peakHours || 'Analysis'}</p>
                            </div>
                        </div>
                        <p className="text-[8px] font-bold text-white/30 uppercase tracking-[0.2em] leading-relaxed">Occupation critique détectée • <br />Optimisation recommandée</p>
                    </div>
                </div>
                ...
                <div className="lg:col-span-4 h-full">
                    <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-12 relative overflow-hidden h-full shadow-3xl group">
                        <h3 className="text-xl md:text-2xl font-black text-white uppercase tracking-tighter mb-10 md:mb-14 flex items-center gap-3 md:gap-4 relative z-10">
                            ...
                            {(stats?.activityStats || [
                                ...
                                <div key={item.label} className="space-y-3 md:space-y-4 group/item">
                                    <div className="flex justify-between items-end px-1">
                                        <div className="space-y-1">
                                            <span className="text-[9px] md:text-[11px] font-black text-white/40 uppercase tracking-widest group-hover/item:text-padel-blue transition-colors">{item.label}</span>
                                        ...
                                        <span className="text-2xl md:text-3xl font-black text-white tracking-tighter group-hover/item:scale-110 transition-transform duration-500">{item.val}%</span>
                                    </div>
